...
// Student view: badge dot only, no expandable flags
const AiBadgeDot = ({ badge }) => {
    if (!badge) return null;
    const config = {
        green: { color: '#22c55e', label: '🛡️ AI Verified' },
        amber: { color: '#f59e0b', label: '⚠️ Flagged for Review' },
        red:   { color: '#ef4444', label: '🚨 Authenticity Query' },
    }[badge];
    if (!config) return null;
    return (
        <span style={{
            fontSize: '0.68rem', fontWeight: '700', color: config.color,
            background: `${config.color}18`, border: `1px solid ${config.color}40`,
            borderRadius: '4px', padding: '2px 7px', marginLeft: '0.5rem',
        }}>
            {config.label}
        </span>
    );
...
const MyClaimsList = ({ student, claims, onToggleVisibility, credentialRequests, setCredentialRequests }) => {
    ...
    return (
        <div className="cv-card">
                            ...
                            <div key={c.id} className={`cv-claim-row ${c.status === 'revoked' ? 'revoked' : ''}`}
                                ...
                                <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '0.25rem' }}>
                                        <span style={{ fontWeight: '600', color: c.visible ? '#fff' : 'var(--cv-text-dim)' }}>{c.type}</span>
                                        {/* Student sees badge label only — no flags */}
                                        <AiBadgeDot badge={c.aiBadge} />
                                    </div>
            ...
            <div className="cv-legend" style={{ marginTop: '2rem', padding: '1rem', borderTop: '1px solid var(--cv-border)', fontSize: '0.8rem', color: 'var(--cv-text-muted)', display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
                ...
                <span>🛡️ AI Verified = passed forensic check</span>
            </div>
        </div>
    );
};
