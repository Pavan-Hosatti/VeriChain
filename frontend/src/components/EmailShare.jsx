...
const EmailShare = ({ student, claims, onShareLogged }) => {
    ...
    const handleShare = async (e) => {
        ...
        try {
            const payload = await buildShareProof({
                student,
            ...
            const token = encodeShareToken(payload, 'magic');
            const magicLink = `https://verify.sealit.com/access?token=${token}`;
            ...
            alert(`Could not create the magic link: ${err.message}`);
        } finally {
            setSending(false);
        }
    };
