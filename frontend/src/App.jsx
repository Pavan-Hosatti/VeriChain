...
import GlobalVerify from './components/GlobalVerify';
import ApiKeyManager from './components/ApiKeyManager';
import { peraWallet } from './services/WalletService';
import { loadCachedClaims, syncOfflineCachesFromClaims } from './services/OfflineVerificationService';
import RevocationService from './services/RevocationService';
import { Shield, Zap, Mail, History, Lock, Eye, ChevronRight, CircleCheck, CircleAlert, UserCheck, Search } from 'lucide-react';
import './index.css';
...
function App() {
    ...
    const [claims, setClaims] = useState(() => loadCachedClaims(MOCK_CLAIMS));
    ...
    const [revocationSyncedAt, setRevocationSyncedAt] = useState(null);
    ...
    useEffect(() => {
        syncOfflineCachesFromClaims(claims);
        ...
        claims.forEach(c => {
            fetch('http://localhost:4001/api/v1/store-credential', {
                ...
                body: JSON.stringify({
                    ...
                    studentName: students.find(s => s.id === c.studentId)?.name || 'Unknown',
                    docType: c.type,
                    value: c.value,
                    issuer: c.issuer,
                    issuedAt: c.date,
                    txId: c.txId || `mock-tx-${c.id}`, // Add a mock txId for demo searching
                    status: c.status,
                    documentHash: c.secureHash || 'mock-hash'
                })
            }).catch(() => {});
        });
    }, [claims]);

    // Poll local cache for revocation snapshot timestamp (updates when RevocationService syncs)
    useEffect(() => {
        const refresh = () => {
            try {
                // loadOfflineCaches is synchronous and returns revocationSnapshotAt
                const caches = loadCachedClaims ? loadCachedClaims() : [];
                // use OfflineVerificationService.loadOfflineCaches to get timestamp
                // avoid direct import cycle by reading localStorage directly
                const ts = window.localStorage.getItem('rapidauth.offline.revocationSnapshotAt');
                setRevocationSyncedAt(ts);
            } catch (e) { }
        };
    ...
    // Start revocation snapshot sync on app startup and schedule periodic updates
    useEffect(() => {
        const baseUrl = import.meta.env.VITE_API_BASE || '';
        // default to hourly sync in demo; production may use 24h
        const intervalMs = parseInt(import.meta.env.VITE_REVOCATION_SYNC_MS || String(60 * 60 * 1000), 10);
        const timerId = RevocationService.scheduleRevocationSync(intervalMs, baseUrl);
        return () => {
    ...
    const handleClaimIssued = (newClaim) => {
        ...
        fetch('http://localhost:4001/api/v1/store-credential', {
            ...
            body: JSON.stringify({
                ...
                docType: newClaim.type,
                value: newClaim.value,
                issuer: newClaim.issuer,
                issuedAt: newClaim.date,
                txId: newClaim.txId,
                status: 'active',
                documentHash: newClaim.secureHash
            })
        }).catch(err => console.error('Failed to sync credential to global API store:', err));
    };
    ...
    if (path.startsWith('/verify/')) {
        const urlId = path.split('/verify/')[1];
        return (
            <div className="cv-app">
                <header className="cv-header">
                    <div className="cv-logo-group" style={{ cursor: 'pointer' }} onClick={() => window.location.href = '/'}>
                        <Shield size={32} color="var(--cv-primary)" />
                        <h2 className="text-gradient">VeriChain <span className="cv-tag">Gateway</span></h2>
                    </div>
                </header>
                <main className="cv-main" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
                    <GlobalVerify initialId={urlId} />
                </main>
            </div>
        );
    }
            ...
            <header className="cv-header">
                ...
                {address && (
                    <div className="cv-header-right">
                        ...
                        <div style={{ marginLeft: '1rem', fontSize: '0.85rem', color: 'var(--cv-muted)' }}>
                            <div>Revocations: {revocationSyncedAt ? new Date(revocationSyncedAt).toLocaleString() : 'Never'}</div>
                        </div>
                    </div>
                )}
            </header>
                    ...
                    <div className="cv-portal-wrap">
                        ...
                        <div className="cv-portal-hero" style={{ textAlign: 'center', marginBottom: '4rem' }}>
                            ...
                            <h1 className="text-gradient" style={{ fontSize: '3.5rem', lineHeight: '1.1', marginBottom: '1.5rem' }}>
                                VeriChain: Immutable<br />Academic Trust
                            </h1>
                            <p className="cv-hero-sub" style={{ fontSize: '1.2rem', opacity: '0.8' }}>
                                Beyond certificates. We're building a source-of-truth ecosystem for 
                                <strong> Enterprise Verifiable Data</strong> where credentials are tamper-proof,
                    ...
                    <div className="cv-dashboard">
                        <aside className="cv-sidebar">
                            <div className="cv-logo" style={{ padding: '0 1rem', marginBottom: '2rem' }}>
                                ...
                                <span style={{ fontSize: '1.25rem', fontWeight: '800' }}>VeriChain</span>
                            </div>
                            ...
                            <div className="cv-nav-group">
                                ...
                                {userRole === 'verifier' && (
                                    <>
                                        ...
                                        <button className={`cv-sidebar-btn ${step === 3 ? 'active' : ''}`} onClick={() => setStep(3)}><Search size={18} /> Global Verify</button>
                                        <button className={`cv-sidebar-btn ${step === 4 ? 'active' : ''}`} onClick={() => setStep(4)}><Lock size={18} /> API Keys</button>
                                    </>
                                )}
                            </div>
                        </aside>
                        ...
                        <div className="cv-content">
                            {userRole === 'issuer' && (
                                <>
                                    {step === 1 && <ContractCaller address={address} students={students} onClaimIssued={handleClaimIssued} issuers={issuers} claims={claims} onRevoke={handleRevoke} credentialRequests={credentialRequests} setCredentialRequests={setCredentialRequests} />}
                            ...
                            {userRole === 'student' && currentStudent && (
                                <>
                                    {step === 1 && <MyClaimsList student={currentStudent} claims={claims.filter(c => c.studentId === currentStudent.id)} onToggleVisibility={handleToggleVisibility} credentialRequests={credentialRequests} setCredentialRequests={setCredentialRequests} />}
                                ...
                                <>
                                    ...
                                    {step === 2 && (
                                        <div className="cv-card">
                                            ...
                                            <p className="cv-hint">Real-time trail of verification events on the distributed ledger.</p>
